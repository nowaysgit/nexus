...
5.  **При архитектурных изменениях - ОБЯЗАТЕЛЬНО создать задачи внедрения:**
    ...
    *   🔄 **Новый паттерн/подход** → **ОБЯЗАТЕЛЬНО** создать подзадачи:
        ...
        *   **ПОДЗАДАЧА РЕФАКТОРИНГА**: "Рефакторить существующий код под [Pattern]"
...
4.  **ПРИНЦИП НЕМЕДЛЕННОГО ЗАМЕЩЕНИЯ И УПРАВЛЯЕМОГО УДАЛЕНИЯ**: 
    *   **НЕМЕДЛЕННОЕ ЗАМЕЩЕНИЕ**: Новая реализация **СРАЗУ** занимает основное имя компонента (например, `CharacterService`)
    *   **LEGACY ПЕРЕИМЕНОВАНИЕ**: Старая реализация **ВРЕМЕННО** переименовывается с суффиксом "Legacy" (например, `CharacterServiceLegacy`)
    *   **АВТОМАТИЧЕСКОЕ СОЗДАНИЕ ЗАДАЧ МИГРАЦИИ**: Интеграционный Менеджер **ОБЯЗАТЕЛЬНО** создаёт подзадачи для:
        - Замены всех использований Legacy-сервиса на новый
        - Валидации корректности работы во всех точках интеграции
        - Полного удаления Legacy-кода после подтверждения стабильности
    *   **МАКСИМАЛЬНЫЙ СРОК LEGACY**: 1 цикл задач. **ЗАПРЕЩЕНО** длительное сосуществование без чёткого плана миграции
    *   **СТРОГИЙ КОНТРОЛЬ**: Интеграционный Менеджер **НЕ ДОПУСКАЕТ** завершения задач с частичной интеграцией или неиспользуемыми Legacy-компонентами
...
*   Провал хотя бы одного теста при запуске полного набора тестов (`yarn test:all --detectOpenHandles --forceExit` или аналогичные команды).
...
#### КРИТИЧЕСКОЕ ПРАВИЛО: Полное выполнение задач исправления ошибок

**АБСОЛЮТНЫЙ ЗАПРЕТ НА ЧАСТИЧНОЕ ВЫПОЛНЕНИЕ КРИТИЧЕСКИХ ЗАДАЧ ИСПРАВЛЕНИЯ:**

Для задач типа "исправить N ошибок ESLint/TypeScript/тестов" действуют следующие **БЕЗУСЛОВНЫЕ ТРЕБОВАНИЯ**:

1. **100% ВЫПОЛНЕНИЕ ЦЕЛИ**: Если задача "исправить 295 ошибок ESLint", должны быть исправлены **ВСЕ 295 ошибок**, а не 85 из 295.

2. **ЗАПРЕТ СТАТУСА `completed` ПРИ ЧАСТИЧНОМ РЕЗУЛЬТАТЕ**: Задача **НЕ МОЖЕТ** быть помечена как `completed`, если цель достигнута не полностью.

3. **ОБЯЗАТЕЛЬНЫЕ ДОПОЛНИТЕЛЬНЫЕ ПОДЗАДАЧИ**: При невозможности завершения в рамках изначальных подзадач **НЕМЕДЛЕННО** создавать дополнительные подзадачи до полного достижения цели.

4. **ЗАПРЕТ ПРОПУСКА ПОДЗАДАЧ**: **КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО** использовать статус `skipped` для подзадач КРИТИЧЕСКИХ задач исправления по любым "стратегическим соображениям".

5. **АВТОМАТИЧЕСКОЕ СОЗДАНИЕ ПРОДОЛЖАЮЩИХ ЗАДАЧ**: При обнаружении задач со статусом `completed`, но с неполным результатом, **НЕМЕДЛЕННО** создавать новую КРИТИЧЕСКУЮ задачу на доработку.

**ПРИМЕР ПРАВИЛЬНОГО АЛГОРИТМА:**
- Задача: "Исправить 295 ошибок ESLint"
- Результат: Исправлено 85 ошибок, осталось 210
- **ПРАВИЛЬНОЕ ДЕЙСТВИЕ**: Изменить статус на `partially_completed`, создать TASK-NEW-CRITICAL "Исправить оставшиеся 210 ошибок ESLint"
- **НЕПРАВИЛЬНОЕ ДЕЙСТВИЕ**: Пометить как `completed` и считать задачу завершенной
...
**КРИТИЧЕСКОЕ ПРАВИЛО: Строгие требования к завершению архитектурных задач**

**КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО** завершать задачи со статусом `completed`, если:

1. **Созданы новые компоненты с "дурацкими" суффиксами** ("Optimized", "Enhanced", "Improved", "New", "V2", "Advanced")
2. **Новые компоненты не интегрированы во все релевантные части системы**
3. **Сосуществуют старые и новые реализации без чёткого плана удаления legacy**
4. **Отсутствуют подзадачи для полной миграции и удаления устаревшего кода**

**ОБЯЗАТЕЛЬНЫЙ АЛГОРИТМ для архитектурных изменений:**

1. **Новая реализация получает ОСНОВНОЕ имя** (например, `CharacterService`)
2. **Старая реализация ВРЕМЕННО переименовывается** с суффиксом "Legacy" (например, `CharacterServiceLegacy`)
3. **НЕМЕДЛЕННО создаются подзадачи:**
   - Интеграция нового компонента во все модули
   - Замена всех использований Legacy на новый
   - Валидация работы во всех точках
   - **Полное удаление Legacy после подтверждения стабильности**
4. **Максимальный срок Legacy**: 1 цикл задач
...
#### КРИТИЧЕСКОЕ ПРАВИЛО: Обязательное завершение ответов с AWAIT NEXT MOVE

**АБСОЛЮТНО ОБЯЗАТЕЛЬНОЕ ТРЕБОВАНИЕ**: При наличии любых невыполненных задач (статус `pending` или `in_progress` в `state/agent-tasks.json`) агент **ВСЕГДА БЕЗ ИСКЛЮЧЕНИЙ** должен завершать свой ответ строкой `AWAIT NEXT MOVE`. Это правило имеет наивысший приоритет и **НЕ МОЖЕТ** быть нарушено ни при каких обстоятельствах.

**ЗАПРЕЩЕННЫЕ ФОРМУЛИРОВКИ**:
- "ПРОДОЛЖАЮ РАБОТУ согласно принципу непрерывности..."
- "Продолжаю выполнение следующих задач..."
- "Переходим к следующему этапу..."
- Любые другие фразы вместо `AWAIT NEXT MOVE`

**ЕДИНСТВЕННЫЙ ФОРМАТ**: Последней строкой ответа должна быть **ТОЛЬКО** `AWAIT NEXT MOVE` без дополнительного текста, точек или других символов.

**ИСКЛЮЧЕНИЕ**: Строка `AWAIT NEXT MOVE` НЕ используется только при **полном завершении проекта**, когда все задачи завершены, все метрики качества достигли целевых значений и отсутствуют любые ошибки.
