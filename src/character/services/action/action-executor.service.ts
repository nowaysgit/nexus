...
export class ActionExecutorService extends BaseService implements OnModuleInit {
  ...
  /**
   * Определяет действие на основе триггера
   */
  private async determineActionFromTrigger(
    ...
    return this.withErrorHandling(
      'определении действия по триггеру',
      async (): Promise<CharacterAction | null> => {
        // Простая логика определения действия на основе типа триггера
        switch (context.triggerType) {
          case 'user_message':
            return await this.generateCommunicationAction(
              character,
              'Ответ на сообщение пользователя',
              context.userId?.toString(),
            );
          case 'emotion_trigger':
            return await this.generateEmotionalAction(character, 'neutral', 5);
          case 'system_event':
            return this.createActionWithResources(character.id, ActionType.SEND_MESSAGE, {
              description: 'Системное действие',
            });
          default:
            this.logDebug(`Неизвестный тип триггера: ${context.triggerType}`);
            return null;
        }
      },
    );
