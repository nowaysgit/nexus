...
export class TechniqueExecutorService extends BaseService {
  ...
  async executeTechnique(
    ...
    return this.withErrorHandling('выполнении манипулятивной техники', async () => {
      ...
      const generatedResponse = await this.generateTechniqueResponse(strategy, context, intensity);

      // Проверяем, была ли ошибка LLM
      if (generatedResponse === 'Техника не может быть выполнена из-за ошибки') {
        return {
          success: false,
      ...
      this.logDebug(`Выполнена техника ${techniqueType} для персонажа ${context.character.id}`, {
        techniqueType,
        intensity,
        effectiveness,
        ethicalScore,
  ...
  async adaptTechniqueToProfile(
    ...
    return this.withErrorHandling('адаптации техники к профилю персонажа', async () => {
      // Получаем персонажа
      ...
      // Адаптируем интенсивность к личности персонажа
      ...
      // Рекомендуем следующую технику
      const nextTechnique = this.recommendNextTechnique(techniqueType, context, needs);

      // Выполняем технику с адаптированными параметрами
      const result = await this.executeTechnique(
        techniqueType,
        adaptedIntensity,
      ...
      // Обогащаем результат информацией об адаптации
      result.message = `Техника адаптирована к профилю персонажа: ${result.message}`;
      result.appliedTechnique = result.appliedTechnique || techniqueType;

      // Добавляем дополнительную информацию об адаптации
      (result as any).adaptedIntensity = adaptedIntensity;
      (result as any).nextRecommendedTechnique = nextTechnique;

      this.logDebug(`Техника ${techniqueType} адаптирована для персонажа ${characterId}`, {
        originalIntensity: TechniqueIntensity.MODERATE,
        adaptedIntensity,
        nextTechnique,
      });
  ...
  private async generateTechniqueResponse(
    ...
    try {
      ...
      this.logError('Ошибка при генерации ответа через LLM', error);
      return 'Техника не может быть выполнена из-за ошибки';
    }
  }
  ...
  async checkEthicalLimits(
    ...
    return this.withErrorHandling('проверке этических ограничений техники', async () => {
      // Проверяем базовые этические ограничения
      const basicCheck = await this.checkEthicalConstraints(techniqueType, context);
      if (!basicCheck.allowed) {
        return basicCheck;
      }

      // Проверяем уязвимость пользователя
      const vulnerabilityScore = await this.assessUserVulnerability(context);
      if (vulnerabilityScore > 80) {
        return {
          allowed: false,
          reason: 'Пользователь находится в уязвимом состоянии',
        };
      }

      // Проверяем интенсивность техники
      if (intensity === TechniqueIntensity.AGGRESSIVE && vulnerabilityScore > 60) {
        return {
          allowed: false,
          reason: 'Агрессивная техника не может быть применена к уязвимому пользователю',
        };
      }
  ...
  /**
   * Оценка уязвимости пользователя
   */
  private async assessUserVulnerability(context: ITechniqueContext): Promise<number> {
    // Базовая оценка уязвимости на основе контекста
    let vulnerabilityScore = 30; // Базовый уровень

    // Учитываем эмоциональное состояние
    if (context.emotionalState) {
      const emotionalStates = ['depressed', 'anxious', 'confused', 'desperate'];
      if (emotionalStates.includes(context.emotionalState.primary)) {
        vulnerabilityScore += 30;
      }

      // Учитываем интенсивность эмоций
      if (context.emotionalState.intensity && context.emotionalState.intensity > 70) {
        vulnerabilityScore += 20;
      }
    }

    // Учитываем уровень отношений (низкие отношения = выше уязвимость)
    if (context.relationshipLevel < 30) {
      vulnerabilityScore += 20;
    }
