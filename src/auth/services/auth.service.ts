...
export class AuthService {
  ...
  async login(loginDto: LoginDto): Promise<{ access_token: string; user: UserResponse }> {
    ...
    if (!user || !(await bcrypt.compare(password, user.password))) {
      this.logService.warn(`Неудачная попытка входа для пользователя: ${username}`);
      throw new UnauthorizedException('Неверные учетные данные');
    }

    this.logService.log(`Успешный вход пользователя: ${user.username} (ID: ${user.id})`);
  ...
  async register(registerDto: RegisterDto): Promise<{ access_token: string; user: UserResponse }> {
    ...
    if (existingUser) {
      this.logService.warn(
        `Попытка регистрации с уже существующими данными: ${username}, ${email}`,
      );
